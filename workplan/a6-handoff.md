...
The values-criteria mechanism: where two populations have evidence-supported preferences that conflict (e.g., visitability framework's zero-step at residential entrances vs. some PAIN-population evidence supporting textured threshold landings), how is the conflict resolved at synthesis time? Person Mode OT co-design resolves at the individual scale; what governs at Population Mode (population-informed)? A6 specifies the mechanism. Per mission §3, "where they diverge, the divergence is documented and a synthesis approach is specified per parameter" — A6 operationalizes "synthesis approach."
