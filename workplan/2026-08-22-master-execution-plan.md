...
| # | Act | Gate |
|---|---|---|
| **R1** | **SETTLED 2026-08-24 by owner ruling, not by adjudication.** The ".md → table" instruction **was never an owner directive** — *"that was machine bundled without my notice. The evening one holds."* So there was never a tension between two directives; there was one ruling and one machine artefact I mistook for a second. Migration 061 cites an owner directive **that does not exist**. Recorded in `decisions/DR-2026-08-24-scaffolding-is-phase-specific.md` | **DONE** — DR written, PROPOSED |
| **R2** | **VOID — mis-framed.** *"we are supposed to be storing things in columns/fields. why are we talking about text strings?"* `authors`, `pub_year`, `title`, `tier_claimed` are **fields**, already columns on `source_locators`. There is no text-blob to place; the markdown-clues-document proposal is **struck** — it would turn structured fields back into prose | **CLOSED** |
| **R3** | **VOID** — depended on R2 going the document way. It did not. The `.md` stays deleted; its fields are columns | **CLOSED** |
...
| **R6** | **~~Delete the 11 unread views~~ — REFUSED ON EVIDENCE 2026-08-25. DO NOT EXECUTE.** The sweep this order required is what refutes it, on two independent grounds. **(a) Empty is not dead:** 8 of the 11 return nothing because their SUBJECT tables are empty — `specifications`, `source_value_extractions`, `spec_value_probes`, `external_root_registry` — deliverable tables awaiting judgment and synthesis. They are render surfaces waiting for data. CLAUDE.md rule 4 already says *"treat a 0-row object as unproven, not clean"*, and that cuts both ways: unproven means do not assume it is dead either. **(b) A cross-stage view IS the pointer** (owner ruling 2026-08-25). `v_source_admission` (evidence-collection ← research) is PD-6's designated pointer target and the remedy for `evidence_sources.search_queries_used`, which is 10/10 populated with a RESEARCH fact on an EVIDENCE row. `v_item_provenance` (evidence-collection ← judgment) is the render→evidence citation pointer the owner described in words, and migration 064 paid to repair it on 2026-08-24. Dropping these removes the pointer infrastructure the ruling mandates and forces the next reader back to copying. **Only `v_coverage_priority` (7,208 rows, research-stage only, no cross-stage role, no reader) is a real candidate — and it is HELD**, because the cross-product ruling of the same day is exactly the shape it serves. | **VOID — do not obey** |
