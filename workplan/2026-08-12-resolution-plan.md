...
wave structure, its sequencing rule and most of its content. **Revision 2** (this one) acts on an
independent read-only adjudication of the whole document set, which found that **revision 1
silently dropped nine findings from its predecessors — two of them classified BLOCKER by the
document that raised them — and carried four arithmetic errors.** §0.3 logs every change from PR
#93; **§0.4 logs every correction made in revision 2**, so both hops are auditable.
...
**Subject:** `888dc60`. **Doctrine SHA:** `0f2f525`.

> **The finding that governs this revision.** Four generations of consolidation ran in 78 minutes
> — findings → register → re-register → plan — and **each hop lost findings while claiming to
> supersede its predecessor.** The clearest casualty: the register paired "correct the five bad
...
Two trials, an audit and a repository-wide sweep sharpen that into **four** statements:
...
**Sequencing rule, unchanged:** fix the substrate, then rule on the boundary, then build. Wave 3
writes rows, and rows are what make Wave 1 expensive.

**Two rules added by rewriting:** *before building a detector, check whether one exists* (W0.1 and
`db.py` are the same lesson twice) — and, from revision 2, ***consolidation without a loss-audit is
how findings die***. Every future supersession must list what it dropped.
...
### 0.4 Corrections made in revision 2

**Nine findings restored** (each was live in a predecessor and absent from revision 1 — verified by
grep returning 0 hits):

| Restored as | Finding | Was live in | Classified |
|---|---|---|---|
| **W1.5** | `pip install -r requirements.txt` fails; header falsely claims two deps suffice | register:107–108 | **BLOCKER** |
| **W1.6** | CLAUDE.md §10 names `session_pointer_resolvable`, a blocking check that does not exist | register:136, consolidated E4:205 | DEFECT |
| **W5.6** | Blocking reproducibility gate compares 93 of 4,245 rows — **2.2%** — and `jurisdictional_values` is not among them | register:121, consolidated C8:186 | **BLOCKER** |
| **W2 / D-C** | Branch protection as an affirmative decision, not only a "don't bundle it" warning | register step 8:240 | DECISION |
| **W5.7** | The renderer makes evidence-thin populations disappear — doctrinal breach in shipped code | register R-21:130 | DECISION |
| **W5.8** | The nine standing advisory failures (`validate_reasoning`, `site_pages_fresh`, `room_page`, …) | register R-26:145 | BACKLOG |
| **W3.9** | The 16-column locator block written out three times — 48 columns for one concept | consolidated F3:216 | DEFECT |
| **W6.8** | The convention-vs-enforcement gap detector proposal | ledger:364–383 | proposed |
| **W7.11** | `references/methodology/` un-finished split; `working/` (39 files) in no register | sweep:266–268, 280–283 | BACKLOG |

**Four arithmetic errors fixed**, each re-derived:

| # | Was | Is | Verification |
|---|---|---|---|
| 1 | W7.3 "−8 tables" | **−6** | G1 −1, G2 −1, G3 −2, G4 −1, G5 −1. The −8 double-counted W7.9's cuts |
| 2 | Net "66 → 56 tables" | **66 → 58** | 66 − 6 folds − 2 cuts |
| 3 | W5.1 "5 rows across 3 items" | **6 rows across 4 items** | `SELECT` returns E-12, B-10, G-04×2, E-07×2 — 6 rows, items B-10/E-07/E-12/G-04 |
| 4 | W7.8 "~34,000 lines" of workplan | **~31,200** | `cat workplan/*.md \| wc -l` = 31,189. The 34,000 was the *script*-line figure from §Net, cross-contaminated |

**One namespace defect acknowledged.** The consolidated review minted Class IDs `A1`–`F8` that
collide with the remediation register's `A1`/`C1`/`D1`, the locator probes' `C1`, and both existing
`F`-series — reintroducing, at generation 3, the exact defect the generation-2 register was created
to fix. **This plan uses only `W`-ids, which collide with nothing.** The Class IDs are cited nowhere
outside their own document, so the rename stays cheap; W7.12 handles it.
...
outputs, so even a machine ruling has nothing to reconcile until Wave 3 lands. Decide now, implement
after.
...
### D-C · Branch protection *(RESTORED — D-OP, owner)*
...
**Revision 1 kept only the negative half of this** ("don't bundle promotions with protection") and
dropped the affirmative recommendation. Both halves are the decision.
...
`evidence_population_match` → `reasoning_doc_citations`; stages 8 and 9 each read all three, and
...
| **W5.4** | **5 of 76 attestations fail CHECK 3**, citing 9 unresolvable ids — and **only `integrity-protocol` is a skill.** The other eight are governance rule names (`retire-not-delete`, `commit-msg-format`, `forward-only-migrations`, `doctrine-token-on-synthesis-paths`, `migration-discipline`, `decision-protocol`, `evidence-architecture`, `tier-system`) that `skill-registry.md` has no place for. The valid universe is 60 = 47 skill ids + 13 `EXTRA_RULE_IDS` | **"Register both ids" fixes one ninth.** First decide whether `rules_in_scope` may cite governance rules — a schema question. Then register the 2 missing skills, correct the rest forward-only, and add the whole-corpus check |
...
| **W5.7** | **RESTORED — the renderer makes evidence-thin populations disappear.** A doctrinal breach in shipped code, not a display bug | **Carried unverified** from the remediation register (§1.0h). Re-derive before acting — it is a doctrinal claim about rendering code, the class this repository has most often found overstated |
| **W5.8** | **RESTORED — the nine standing advisory failures**, all reproducing: `validate_reasoning` (the only reasoning doc fails every structural requirement) · `validate_pydantic_schemas` (246 findings, 49 unmapped tables) · `retired_vocabulary` (69 occurrences) · `site_pages_fresh` (12 stale pages) · `research_dod` R1 · `test_verification_pipeline` 15/18 · `test_directness_2_2` · `test_graph_audit` · `register_integrity_check` selftest. Plus `parts/v10` stale in all 15 files and ungated, and `room_page.py` querying six non-existent tables | Backlog. **Do not clear by silencing** — each needs a leg-level resolution, not a check-level skip |
...
| # | Issue | Fix | Status |
|---|---|---|---|
| **W6.1** | The trial has a 105-action verbatim log; the review pronouncing CONFIRMED/OVERSTATED on another session's work has none | Route review work through the harness — `run()` logs argv, cwd, exit code, stdout, stderr and per-table deltas | `walk_harness.py` **DONE** |
...
| **W6.8** | **RESTORED — a convention-vs-enforcement gap detector.** For each convention the repo states anywhere — CLAUDE.md, `project-standards.md`, a skill, a docstring — does a registered check enforce it? Report the stated-but-unenforced set. **Evidence it would pay:** dimensions with an enforcer sit at ~75% compliance (`GUIDEBOOK_DB_PATH` 74%, read-only opens 76%); dimensions without one at ~50%, indistinguishable from no convention | One script against the existing registry. **If it cannot be expressed as a registry check, that is evidence not to build it.** Note the confound: enforcers may have gone to conventions already judged important | proposed |
| **W6.9** | **NEW — every supersession must publish a loss-audit.** Revision 1 of this plan dropped nine findings, two of them BLOCKER, while claiming to reconcile its sources | A document that supersedes another lists what it did not carry forward. §0.4 is the first instance | proposed |
...
| **W7.2** | **Merge the ten single-invariant audit scripts** behind `scripts/audit/invariants.py --check <id>`. The registry dispatches by argument in 27 of 65 entries, so each stays individually registered and quarantinable | −9 files, ~−200 lines | **Drop first if the ~1,000-line result feels like `db.py` again. Two of the ten are quarantined — reconcile with `tooling-register.md` §6.5, which makes quarantine terminal, before touching them** |
| **W7.3** | **Five FK-safe folds:** `bpc_metadata`→`slugs` (both PK `slug`, 1:1) · `citation_mining`→`source_slug_links` (same grain, two key spellings) · `access_duration`+`access_stakes`+`life_stage_modifiers`→one vocabulary table · `case_study_outcomes`+`case_study_strategies`→one child table · `search_coverage`+`search_languages`→one axis column | **−6 tables** *(corrected from −8)* | DDL only, all empty |
...
| **W7.8** | **Generate `workplan/INDEX.md`** — date-sorted, reset-relative status column, wave ids, registered for freshness. **74 workplans, 31,189 lines, no index.** The rename was adjudicated FATAL; the index was adopted in its place and never built | +1 generated file | none |
...
| **W7.11** | **RESTORED — two unregistered surfaces.** `references/methodology/` carries an un-finished split (`economics-research-methodology-v1.9-archived.md` contains four descendant files at 83–98% containment, kept in the live directory); `working/` — 39 files, ~1.1 MB, wholly corpus-derived — is in no register | — | owner |
| **W7.12** | **Consolidate this session's own seven documents.** Independent adjudication found ~35–40% of 2,523 lines restate content stated elsewhere, and fifteen findings appear in three to five documents. Target: **four documents** — this plan; `consolidated-review-and-plan.md` (register of record, Part 3 reduced to a pointer here); `consolidation-sweep-and-adversarial-pass.md` (frozen-corpus evidence, Part 3 marked superseded); `per-stage-table-anatomy.md` (unchanged). **Retire with redirect stubs, after porting:** `reconciled-findings-register.md` (its §0.2 namespace table, §2.1 executed refutation and Part 6 near-miss are unique), `pr93-reconciliation-and-shared-code.md` (its R↔W map is the only Rosetta stone between the two ID systems), `fold-or-cut-ledger.md` (its Part 1 phase distribution, §2.6 `v_coverage_priority` 7,210-rows-no-reader, Part 4 bucket census and Part 5 skill tests are unique) | −3 files | owner. **Port before retiring — every prior consolidation in this session lost findings** |
| **W7.13** | **Rename the consolidated review's Class IDs.** `A1`–`F8` collide with the remediation register's `A1`/`C1`/`D1`, the locator probes' `C1`, and two existing `F`-series — the exact defect its predecessor register was created to fix. Cited nowhere outside their own document, so the rename is cheap now and gets dearer | — | none |
| **W7.14** | **Preserve `scratchpad/stages.json` as committed data; discard the rest.** It is the stage→table extraction behind the phase-multiplicity test, the ledger's Part 1 and the r = 0.73 validation — a judgment extraction from a 6,775-line document that none of them reproduces. `allpy.txt`, `gb.txt`, `gb2.txt`, `ga.err`, `purpose.json` and `blk/` are all mechanically reproducible | +1 data file | none |

**Not recommended, each adjudicated before:** bulk renames (K3); deleting a quarantined script
(`tooling-register.md` §6.5 — and W0.1 shows the list has value); promoting checks in the same
window as branch protection (K4); folding the three population-link tables or the 15 substantive
audit scripts; touching the 49 skills, which survived three independent tests with nothing cuttable.
...
| W7.3 five folds | −6 tables | — |
| W7.9 cuts | −2 tables, −2 views | — |
| W7.12 document consolidation | −3 | ~−900 |
| Injection (2 seams, both into `db.py`) | **0** | ~−150 |
| W7.4 if `db.py` is not adopted | −1 | −1,889 |

**Net: 133 → 105 executables · ~33,700 script lines (−16%) · 66 → 58 tables · 18 → 16 views.**
...
of script code.** Injection is a *consistency* measure and must be argued only on its two correctness
grounds: W1.4's schema violation and the read-only handle. Applying "fewer files" honestly **drops
two of the four seams originally proposed** — `report()` and `repo_root()` each need a module *and* a
check, for style conventions with no defect history, and `report()`'s one verdict-changing convention
...
W5 (corpus) — independent, except W5.3 (waits on W4.5) and W5.1 (pairs with W5.6)
W6 (method) — independent; W6.1-W6.3 done
W7 (consolidation) — independent, except W7.1 (closes Wave-1 writers),
                     W7.4 (begun by W1.4), W7.12 (port before retiring)
```
...
runs against an empty corpus. Treat the empty window as cheap *and* under-tested.
...
W0.1 is minutes: a registry entry for a script that already works and already names six wrong rows
in the canonical database. It is the only item here that *finds* defects rather than preventing them.

W1.1 is four lines. A repository whose cardinal rule is *never write the database directly* commits
foreign-key violations through its own sanctioned write path and reports them as errors. Everything
else assumes the substrate holds.

**And W5.1 does not ship without W5.6.** Correcting six values in a table the blocking gate does not
watch leaves nothing to notice them coming back — which is the defect this plan opens by describing.

---

*Every figure re-derived on 2026-08-11 against `888dc60` by the command quoted beside it. Revision 2
corrected four arithmetic errors and restored nine findings its own revision 1 dropped; the
verifications are in §0.4. Counts of the database, the check suite and CI are volatile — the
`run_checks --all` total moved within a single session because attestation-scoped checks read the
git changeset. Re-derive before acting, and note W6.6: four proxy measurements inflated results
across the sessions that produced this plan.*
