...
> **Methodological standard — Empirical derivation from user research [GAP-CON-06 / CON-0015]:** All specifications in Part 4 are derived empirically from the needs of disabled users — from OT clinical research (Tier 1), lived experience evidence (Co-1), participatory design studies (Co-1), systematic reviews (Tier 3), OT professional body guidelines (Co-2), and standards (Tier 4–6) — in that order of priority per the evidence hierarchy (project-standards §Core Doctrine). Specifications are NOT derived from code consensus. Code consensus (Tier 6) is the compliance floor, not the design aspiration. Where Part 4 specifications diverge from code, the divergence is intentional — the higher-tier evidence is governing. Designers should understand this as a fundamental difference from a compliance manual: every value in Part 4 has a traceable evidence basis, and that evidence basis governs. The question to ask of every specification is not "does this meet code?" but "does this reflect the most accommodating, dignified, usable condition the evidence supports?" (RULE 2026-04-24 23:46, project-standards §Core Doctrine). This principle applies equally to specifications that match code values — the match is coincidental; the derivation is from evidence.
...
**Cross-room biomechanical constant — Accessible storage envelope [CON-0266]:** All user-accessed storage in accessible dwellings shall be within the 380–1200 mm AFF envelope regardless of room type. Below 380 mm AFF = fall risk from ≥30° trunk lean in seated trunk-unstable users (FDR-BAB-05 bedroom dressing, FDR-KIT-03 laundry — identical biomechanical mechanism documented independently in two rooms). Above 1200 mm AFF = forward reach limit exceeded for seated users (ANSI A117.1; PVA SCI CPG: no overhead reach for bilateral UPL). This envelope appears independently in bedroom, kitchen, laundry, and bathroom FDR runs. Individual room specifications (G-08, I-02, I-03) should cross-reference this universal envelope rather than deriving room-specific storage bounds independently. Mode S adjusts upper bound to individual shoulder height via OT assessment. ● [Four-room convergence on identical biomechanical limits]
...
<!-- SYNTH-SCAN CON-0264: RT60 ≤0.4s multi-population convergence. Five independent population evidence streams converge on the same acoustic specification: (1) DEAF — speech intelligibility in reverberant environments (BS 8300, DIN 18041); (2) NDV/AUT — sensory overload reduction in reverberant spaces (FDR-SRS-01, PAS 6463); (3) MH — de-escalation room acoustic requirement (MH BPC); (4) PAIN — ambient acoustic comfort; reverberant environments increase pain perception (pain-ofs BPC); (5) A-16 sensory room — universal quiet room specification. RT60 ≤0.4s is the cross-population acoustic constant for spaces where sensitive populations are primary users. Individual items (A-02, A-03, A-10b, A-16) should cross-reference this convergence rather than restating the value independently. Note: A-16, speech-critical classrooms, and DEM environments specify the stricter RT60 ≤0.3s; ≤0.4s is the general occupied-space target. -->

**Universal acoustic target [CON-0264]:** RT60 ≤0.4 s is the cross-population acoustic constant for all occupied spaces where sensitive populations (DEAF, NDV/AUT, MH, PAIN, OFS) are primary or foreseeable users. This value derives from five independent population evidence streams converging on the same threshold. Spaces with higher acoustic demand (A-16 sensory rooms, speech-critical classrooms, DEM residential) specify the stricter RT60 ≤0.3 s. The ≤0.4 s target is the general design floor — rooms that achieve ≤0.4 s satisfy the acoustic requirements of all five populations simultaneously without population-specific conditioning. ● [Five-population convergence]
...
~~Staud, R. (2011). *Curr Pain Headache Rep*, 15(5), 338–343.~~ [CLOSED-DELETED — Standing Rule 5: two independent searches (2026-05-05) failed to locate this article at the stated pages/volume/journal. No PMID or DOI found. Citation removed from authoritative basis. GAP-IMPL-05 CLOSED. Evidence basis for central sensitisation mechanism remains supported by Desmeules et al. (2003) above; vibration threshold specification draws on Garcia-Mendez et al. 2013 and ISO 2631-1 per revised A-09 specification.]
...
**Laundry appliance vibration [CON-0270]:** In accessible dwellings, laundry appliances on pedestals (300–400 mm AFF per FDR-KIT-03) transmit WBV through the floor to adjacent seated/wheelchair users. For PAIN/FMS populations, appliance vibration is a pain trigger via the same central sensitisation mechanism as HVAC vibration. Specification: anti-vibration pads beneath appliance pedestals; resilient flooring in the laundry zone; appliance operating position ≥1200 mm from the nearest seated work position or rest seating. Garcia-Mendez 2013 confirms floor type is the single largest WBV determinant — hard tile transmits maximum WBV, while resilient flooring attenuates it. ○ [CONFIDENCE: moderate — mechanism is well-established (PAIN vibration sensitivity + WBV research); 1200 mm separation distance is extrapolated from domestic vibration isolation practice, not directly studied for PAIN populations]
...
**Anti-token quality criteria [SYNTH-SCAN C-01]:** A sensory room that meets any of the following conditions is NOT an A-16-compliant provision and must not be counted toward the one-per-floor-plate requirement: (a) area <4 m² (FDR-SRS-01 absolute minimum; 8 m² is specification minimum — 4 m² is the threshold below which no clinical function is achievable); (b) no natural light access AND no full blackout capability (must have one or both — a windowless room without blackout is a storeroom); (c) background noise >45 dB(A) (acoustic treatment absent — MH evidence: Wilson 2023 Co-1 documents repurposed storerooms with no acoustic treatment as clinically useless); (d) requires advance booking or staff permission to access (on-demand availability is a clinical requirement — NDV FDR; delay negates the acute regulation function); (e) shares a wall with plant room, kitchen, or other high-noise source without STC ≥50 partition. These criteria prevent the documented failure mode of token provision — a room that exists on paper and satisfies a compliance checkbox but fails functionally. ● [Four-source convergence: Wilson 2023 Co-1 MH failure documentation + FDR-SRS-01 quantified physics + Kawa Model B shared calm criteria + NDV FDR on-demand access requirement]
...
**DEM stage-specific wayfinding × VIS convergence [CON-0271]:** DEM wayfinding strategy changes as dementia progresses: early stage uses distinctive landmarks (diversity, individuality at decision points); advanced stage relies on spatial regularity and predictability (consistent layout, repetitive cues). This apparent divergence resolves when stage is considered alongside VIS: (a) early DEM + VIS CONVERGE on distinctive landmarks at decision points — both populations benefit from unique, memorable reference points; (b) advanced DEM + VIS CONVERGE on spatial consistency — both populations benefit from predictable, regular environments. The specification should provide BOTH: landmark diversity at decision points (serving early DEM + VIS) AND spatial regularity in overall layout (serving advanced DEM + VIS). These are not contradictory — landmarks are local exceptions within a globally regular spatial framework. D-01 (loop plan) provides the regular framework; D-04 (landmarks) provides the local distinctiveness. Stage-transition note for residential DEM: as residents progress, the wayfinding support shifts from landmark-based to regularity-based; the physical environment must accommodate both stages simultaneously. ○ [CONFIDENCE: moderate — DEM stage-specific wayfinding is documented (BPC DEM); VIS convergence at both stages is interpolated from this session's synthesis scan]
...
<!-- ve_risk: CRITICAL -->
<!-- ot_appointment_trigger: conditional:MOB -->
<!-- grade_confidence: HIGH — Grab bars bilateral, vertical primary: Levine 2024 PMID:38968649 (J Biomechanics Tier 1) — grasp location varies with height r=0.67; KITE Research 2023-25 (Tier 3) — 1.3 kN load. Kennedy 2015 (Tier 3) — vertical bars = smallest COP deviation. Clemson 2023 Cochrane PMID:36893804 (Tier 1 SR 22 RCTs) — OT home hazard reduction 38% fall reduction. ≥8 jurisdictions mandate grab bars (though ADA specifies horizontal — evidence supports vertical). HIGH on provision; MODERATE on exact configuration (ADA divergence noted). Citation mining 2026-05-04: Lee/Sanford 2018 (ULB-12, HERD 11(2):32-44, DOI:10.1177/1937586717730338) — n=130+ residents, 11 facilities; bilateral fold-down bars at 356mm from toilet CL, 813mm AFF significantly outperform ADA configuration. Most robust grab bar evidence in BPC corpus. Levine 2022 (ULB-14, Front Public Health, DOI:10.3389/fpubh.2022.915100) — Canadian national survey supports mandatory bathroom grab bars (Co-1). -->
...
**Proximal stabilisation surface [CON-0260]:** A horizontal bracing surface at 900–1000 mm AFF shall be provided within arm's reach (≤600 mm) of every user-operated control. Three populations use this feature for different clinical reasons: (a) tremor — forearm bracing before fine-motor activation (FDR-RRC-04, OT prescription pattern); (b) PAIN — incidental environmental bracing during pain episodes; (c) NEU/PCS — postural compensation for impaired balance during standing control operation. Implementation: window sill, counter edge, shallow shelf, or grab rail segment adjacent to switches, thermostats, intercoms, and access control readers. On primary circulation routes, a continuous horizontal rail or ledge at 900–1000 mm AFF at 6 m intervals provides bracing opportunities between rest seating positions (E-10). ○ [CONFIDENCE: high for mechanism (3-population convergence); moderate for 6 m interval (extrapolated from corridor rest seating data, not directly studied)]
...
**Bedroom-bathroom tactile route [CON-0269]:** In residential settings serving DBL users, the TWSI directional guidance system must extend to the bedroom-to-bathroom route — not only public corridor routes. This route is currently unspecified in any standard or FDR. The bedroom-bathroom route requires: (a) TWSI guidance strip from bedroom door to bathroom door (or continuous within an en-suite configuration); (b) attention pattern at bathroom door threshold; (c) Braille signage at bathroom entry confirming room identity. The overnight route is navigated without visual or auditory cues — tactile guidance is the only reliable wayfinding channel for DBL users in darkness. Coordinate with G-09 (overnight pathway lighting — lighting serves DEM/PTSD but does not serve DBL; tactile guidance serves DBL). Tactile map (K-02) should include the bedroom-bathroom route segment in residential building maps. ○ [CONFIDENCE: moderate — extrapolated from FDR-DB-01 general tactile wayfinding principles to specific room pair; no standard addresses this route segment for DBL]
...
**PTSD+DEM bedroom overnight convergence [CON-0267]:** Bed position in NDV/MH or DEM residential rooms must have unobstructed sightline to room exit door. Two populations independently require the same spatial property: (a) MH/PTSD — exit sightline from inside room to exit prevents entrapment perception and anti-entrapment anxiety (Faerden 2022; Wilson 2023 Co-1); (b) DEM — visual access to exit door reduces overnight disorientation and supports independent bathroom access (DSDC EADDAT 2022; D-03 toilet visibility principle). The continuous low-level pathway lighting (300–400 mm AFF, ≥5 lux, 2700–3000 K) specified above satisfies both populations: it provides a visible illuminated route from bed to exit that the DEM user can follow and the PTSD user can visually confirm. This is a compound convergence producing a stronger specification than either population's requirement alone. Bed position is a Layer 3 (fit-out) decision — coordinate with bedroom furniture layout. ● [Two-population convergence; both requirements independently evidence-based. Cross-ref §3.4 Principle 5 (destination visibility)]
