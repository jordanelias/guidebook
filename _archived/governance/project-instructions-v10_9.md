...
**V10.9 · Revised 2026-05-13**
Supersedes V10.8. Architecture: `project-architecture-guidebook-v2.3.md`. Preferences: `userPreferences-v6.1.md`.

<changelog>
- **V10.9 (2026-05-13)** — Standing rule #10 sharpened per DR-2026-05-13 (adopted session_2026-05-13b). Distinguishes existence verification (`verification_status = VERIFIED`) from claim verification, which is now explicitly required by claim type: numerical-spec claims via PMP (rule #8); jurisdiction-comparison cells via new `reasoning_doc_citations` table; qualitative/definitional claims via the same table with `claim_match` field. Migration 007 (FK integrity fix; 4 tables, 782 rows) and Migration 008 (`reasoning_doc_citations` table) applied. user_version 9 → 11. Track 1 versioning backfill on publisher-catalog-fetch records initiated. New skill placeholder: `reasoning-doc-citations` (invoked during Phase E.1). Research log entry recorded with five web searches surveying Cochrane, GRADE, JBI methodologies and the 17-25% peer-reviewed quotation-error base rate (Cobey et al. 2025; Mogull 2017; Jergas & Baethge 2015; Smith & Cumberledge 2020).
...
- `reasoning-doc-citations` — invoked per standing rule #10 sub-rules 2 and 3 during Phase E.1. Records per-cell verification of jurisdiction values, qualitative claims, and definitional claims. Skill placeholder added v10.9 per DR-2026-05-13; `skills/reasoning-doc-citations_SKILL.md` to be authored in Phase A parallel-track.
...
10. **Evidence verification gate for synthesis.** No BPC synthesis claim may cite a source with `metadata_quality = AUTHOR-TITLE-ONLY` or `verification_status = NULL`. Required minimum: `metadata_quality = COMPLETE` AND `verification_status ∈ {VERIFIED, UNVERIFIED-1}`. Sources with `verification_status ∈ {UNVERIFIED-CLOSED, CLOSED-DELETED}` are excluded from all synthesis.
    ...
    **Note on the existence-vs-content distinction (sharpened in v10.9 per DR-2026-05-13).** `verification_status = VERIFIED` confirms source existence and identification, not content. The 17-25% quotation-error base rate in peer-reviewed medical literature (Cobey et al. 2025; Mogull 2017; Jergas & Baethge 2015) is a reasonable lower bound on the rate of content-failures the Guidebook would inherit if existence-verification were used as a content-grade gate. Additional content-verification requirements apply by claim type:
    ...
    2. **For jurisdiction-comparison cells in 9-step reasoning docs** (rule #9 step 3): each (parameter, jurisdiction) cell must have a corresponding `reasoning_doc_citations` row with `value_match ∈ {EXACT, WITHIN-TOLERANCE}` from a documented re-read of the specific code section. `value_match = PAYWALL` requires either a downgraded synthesis grade or a non-paywalled corroborating source. `paywall_purchase_candidate = 1` may be flagged for periodic owner review.
    3. **For qualitative and definitional claims**: the synthesis must additionally cite a `reasoning_doc_citations` row with `claim_type ∈ {qualitative, definitional}` and `claim_match ∈ {SUPPORTED, PARTIAL}` from a documented re-read of the source section. `claim_match = CONTRADICTED` invalidates the claim.

    **Track 3 pilot mandate.** The first BPC entering Phase E.1 under the new `reasoning_doc_citations` workflow runs as a formal pilot. All rows produced under the pilot must be inline-reviewed before Track 3 scales. This is the structural mitigation for the unavailable dual-reviewer (single-reviewer project; see DR-2026-05-13 §2.3).

    Current state at v10.9 adoption (per session_2026-05-13b audit):
    - 457/661 sources are AUTHOR-TITLE-ONLY (69%) — INELIGIBLE for synthesis. Down from 567 at v10.8 adoption; Phase B continues.
    - 258/661 sources have `verification_status = NULL` (39%) — INELIGIBLE for synthesis. Down from 647 at v10.8 adoption.
    - 410/675 records VERIFIED (60.7%) — existence-verified only; content verification governed by sub-rules 1–3 above.
    - 0/675 records have content-verified claim-level linkage. This is the gap the sharpened rule addresses.

    Phase B of the operative workplan rehabilitates these sources (existence). Tracks 1 and 3 per DR-2026-05-13 add claim-level verification. Until Phase B is complete for a given BPC's formally-linked sources AND that BPC's Phase E.1 produces the required `reasoning_doc_citations` rows, that BPC's synthesis is BLOCKED for the relevant claim type.
    ...
    Audit queries:
    - `scripts/audit_evidence_metadata.py` (existence gate; to be written in Phase A.10, level 2 enforcement)
    - `scripts/audit/reasoning_doc_citations_audit.py` (claim-level gate; stubbed v10.9, level 2 enforcement)
    - `scripts/audit/pmp_audit.py` updated to cross-check numerical-spec synthesis against PMP walks (level 2 enforcement)
</standing_rules>
...
P1="?"; PMP_BACKLOG="?"; ATT_ONLY="?"; UNVERIFIED="?"; REASONING_BPC="?"; REASONING_CON="?"; RDC_TOTAL="?"; RDC_PAYWALL="?"; VERS_BACKFILL="?"
if _GET_BIN "data/guidebook.db" /tmp/guidebook.db; then
  ...
  # v10.9: reasoning_doc_citations metrics (Track 3) and versioning backfill (Track 1)
  RDC_TOTAL=$(python3 -c "import sqlite3;c=sqlite3.connect('/tmp/guidebook.db');print(c.execute(\"SELECT COUNT(*) FROM reasoning_doc_citations\").fetchone()[0]) if c.execute(\"SELECT name FROM sqlite_master WHERE type='table' AND name='reasoning_doc_citations'\").fetchone() else print('0')" 2>/dev/null) || RDC_TOTAL="?"
  RDC_PAYWALL=$(python3 -c "import sqlite3;c=sqlite3.connect('/tmp/guidebook.db');print(c.execute(\"SELECT COUNT(*) FROM reasoning_doc_citations WHERE paywall_purchase_candidate=1\").fetchone()[0]) if c.execute(\"SELECT name FROM sqlite_master WHERE type='table' AND name='reasoning_doc_citations'\").fetchone() else print('0')" 2>/dev/null) || RDC_PAYWALL="?"
  VERS_BACKFILL=$(python3 -c "import sqlite3;print(sqlite3.connect('/tmp/guidebook.db').execute(\"SELECT COUNT(*) FROM evidence_sources WHERE superseded_by_ref_id IS NOT NULL OR edition IS NOT NULL\").fetchone()[0])" 2>/dev/null) || VERS_BACKFILL="?"
else
...
if command -v gh >/dev/null 2>&1; then
  ...
  REASONING_BPC=$(curl -fsSL -H "$AUTH" "https://api.github.com/repos/$REPO/contents/references/bpc-reasoning" 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d) if isinstance(d,list) else 0)" 2>/dev/null || echo 0)
  REASONING_CON=$(curl -fsSL -H "$AUTH" "https://api.github.com/repos/$REPO/contents/references/connection-reasoning" 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d) if isinstance(d,list) else 0)" 2>/dev/null || echo 0)
fi
...
echo "Versioning backfilled: $VERS_BACKFILL / 675  (Track 1 progress per DR-2026-05-13)"
echo "reasoning_doc_citations rows: $RDC_TOTAL  (Track 3 progress per DR-2026-05-13)"
echo "PAYWALL candidates flagged: $RDC_PAYWALL  (for owner purchase review)"
...
**v10.9 update notes:**
- Status block extended with three new metrics: versioning backfill count (Track 1), `reasoning_doc_citations` row count (Track 3), and PAYWALL purchase-candidate count (for owner review).
- Bootstrap SQLite queries now check for `reasoning_doc_citations` table existence before counting (forward-compatible if migration 008 hasn't been applied in some checkout).
- Tested: 2026-05-13 cold-start under curl backend. SQLite queries return correct counts including the new Track 1/3 metrics. Runtime ~2-3s. Architecture's <30s budget met.
- v10.9 inherits v10.8's bootstrap structure; only the SQLite query block and status output lines were extended.
</bootstrap>
