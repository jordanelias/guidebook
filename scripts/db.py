...
def main():
    ...
    if args.command == "gaps":
        ...
        # Repointed in the code-only fork. This used to shell out to
        # validate_db.py, which was written against a schema the DB no longer
        # has and crashed on `no such column: doi_less_key`. It was quarantined
        # rather than retired precisely because this call site kept it alive.
        # test_db_integrity.py is the maintained successor and CLAUDE.md
        # already directs people to it.
        import subprocess
        sys.exit(subprocess.call(
            [sys.executable, str(Path(__file__).parent / "tests" / "test_db_integrity.py")]
        ))
