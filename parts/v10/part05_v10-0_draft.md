...
| **ACOUSTIC-LVL — Amplification and background noise** | DEAF, CI users | NDV/SENS, NDV/AUT, NEU/PCS | Hearing loop (IEC 60118-4) in all assembly spaces, counters, and classrooms; STI ≥0.75 for DEAF/CI (Iglehart 2020) | No amplification or broadcast audio in sensory retreat and quiet zones; background noise ≤30 dBA (Zone 3); no sound masking (A-13) | Hearing loop amplifies direct speech pickup without requiring broadcast audio. Audio input to loop system is independently controlled from PA and background music circuits. Loop zone on IEC 60118-4 with dedicated direct-input microphone; PA and broadcast audio muted in Zone 3 areas. RT60 ≤0.3 s (NDV target governs as stricter subset of DEAF ≤0.4 s; speech clarity serves DEAF users; acoustic calm serves NDV). No sound masking in any space where NDV is a primary population. | RESOLVED-CONSENSUS | ◐ IEC 60118-4:2022 (loop zones and field strength); ● Iglehart 2020 AJA (RT60 ≤0.3 s, CI/HA speech scores); ◐ PAS 6463:2022 (NDV no masking). Cross-ref: A-10, A-11, A-13. |
