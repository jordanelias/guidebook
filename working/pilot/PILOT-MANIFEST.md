...
| 3 | G-03 Grab Bars × MOB | `ot-cpg-built-environment` | 0/0/4/2/0/0/0 | Co-2 (OT CPG) + T2 anchoring (§2.2 condition 3). **No pure Co-2-only cell exists in the corpus** — that exact branch is exercised by unit test, recorded honestly here | `stated` |
...
| 6 | G-03 Grab Bars × SCI | `fold-down-grab-bar-specification` | zero linked sources (slug ACTIVE) | `pending` + gap link; Altman & Bland honesty | `pending` |
...
**Branches with no real cell — covered by synthetic rows in mutation tests only (never left in the DB):** pure T6-only `code_floor_only`; `not_applicable` (+ rationale enforcement); forced Co-1/T1 divergence (cell 2's convergence is assessed from real data; whether it diverges is an empirical outcome, not a scripted one).
