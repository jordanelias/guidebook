...
| L1.6 — 12 tables carry a legacy population column with no lens columns | **REPRODUCES, given the base exclusion.** 14 tables carry `population_code`/`population`/`target_population` without the four lens columns; two of them (`populations`, `population_icf_grouping_map`) are base registries the ruling scopes out, leaving **12** |
...
| **L1.6** | A table attaching a determination to a group of disabled people takes four lens columns and a COALESCE CHECK | `PRAGMA table_info` over every non-`base_*` table: presence of `population_code`/`population`/`target_population` **and** absence of all four of `identity_code/icf_code/needs_code/medical_code` → MISS. Exclude the base registries by name — `populations` and `population_icf_grouping_map` are the identity lens and a crossing map, and the ruling scopes to "after the base phase". **14 raw, 12 after the exclusion**; five of the twelve are the splinter tables D-0182 says are deleted, all at 0 rows. Assert the surviving CHECK is `COALESCE(...) IS NOT NULL`, **never** a sum `= 1` | **v1** | ruling unswept |
