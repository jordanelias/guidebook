...
| **L1.3** | A closed vocabulary has exactly one home — the column's own CHECK | For each entry in `emit_data_migration.RANGE_GUARDS`, assert the named column has **no** CHECK in the live DDL. A guard whose column also has a CHECK is a second home (rule 5) — the guard is then the copy, and it fails CLOSED on the next widening while the CHECK moves on. **CORRECTED 2026-09-21: this row named `ENUM_GUARDS` (2 entries) until migrations 091/092 gave both its vocabularies real CHECKs, at which point the requirement as written would have gone red on the CHECKs and invited removing THEM rather than the guard — the inverse of the rule it states. ENUM_GUARDS was deleted instead; `RANGE_GUARDS` is the only remaining entry, and `evidence_sources.tier` genuinely has no CHECK.** See P2: the naive form of this test reports a phantom CHECK on `tier` from a word inside a comment | **v1** | 1 (inverse) |
