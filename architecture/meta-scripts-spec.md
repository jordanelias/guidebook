...
| L1.6 — 12 tables carry a legacy population column with no lens columns | **REPRODUCES, given the base exclusion.** 14 tables carry `population_code`/`population`/`target_population` without the four lens columns; one of them (`populations`) is a base registry the ruling scopes out, leaving **11** — re-derived 2026-09-16 after migration 084 deleted `population_icf_grouping_map`, which had been the second excluded name. Derive, never quote: see the command in the L1.6 row below |
...
| **L1.6** | A table attaching a determination to a group of disabled people takes four lens columns and a COALESCE CHECK | `PRAGMA table_info` over every non-`base_*` table: presence of `population_code`/`population`/`target_population` **and** absence of all four of `identity_code/icf_code/needs_code/medical_code` → MISS. Exclude the base registries by name — `populations` is the identity lens, and the ruling scopes to "after the base phase". (`population_icf_grouping_map` was the second excluded name until migration 084 deleted it, 2026-09-16.) **Re-derived 2026-09-16: 12 raw, 11 after the exclusion, and all 11 at 0 rows.** Never quote that pair — it moved twice in one week. Derive it:<br>`python3 - <<'PY'`<br>`import sqlite3; c=sqlite3.connect('file:data/guidebook.db?mode=ro',uri=True)`<br>`L={'identity_code','icf_code','needs_code','medical_code'}; P={'population_code','population','target_population'}`<br>`for (t,) in c.execute("select name from sqlite_master where type='table' order by name"):`<br>`&nbsp;&nbsp;cols={x[1] for x in c.execute('PRAGMA table_info(\"%s\")'%t)}`<br>`&nbsp;&nbsp;if (cols&P) and not (cols&L): print(t, c.execute('select count(*) from \"%s\"'%t).fetchone()[0])`<br>`PY` Assert the surviving CHECK is `COALESCE(...) IS NOT NULL`, **never** a sum `= 1` | **v1** | ruling unswept |
