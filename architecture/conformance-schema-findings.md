...
| L1.3 | A closed vocabulary has exactly one home — the column's CHECK | `dbcore.check_values()` non-empty for every column a writer validates; each `RANGE_GUARDS` entry must name a column whose CHECK is absent, else it is a second home. **CORRECTED 2026-09-21: `ENUM_GUARDS` was named here until 091/092 gave both its vocabularies real CHECKs — at which point this requirement, as written, would have gone red on the CHECKs and invited removing them rather than the guard. The guard was deleted instead.** | 1 (inverse) |
