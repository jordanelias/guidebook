...
- Status: **APPLIED, but BLAST RADIUS INCOMPLETE — see `DR-2026-07-14-person-mode-governance-reconciliation.md`.** This DR corrected three spots (mission commitment 4 — applied, not "held"; `evidence-architecture.md` §3 + §5). A later adversarial Fable-5 audit found the rejected "within the range" framing survived in 10+ further canonical/forward-looking locations, including inside the two files edited here (`mission-and-epistemics.md:107`, `evidence-architecture.md` §8.1/§8.4) and the canonical home of decision D-D (`evidence-methodology.md` §1.6). The canonical-governance remainder is reconciled in DR-2026-07-14; skills / locked RULEs / armature are deferred there. The "three canonical spots" / "corrected atomically" completeness claim in this DR is therefore superseded and should not be read as a full blast-radius record.
- Date: 2026-07-13
...
- Affects: `governance/mission-and-epistemics.md` commitment 4 table (doctrine-SHA anchor — **applied** in the same commit, not held; the "held" wording here was stale plan-stage text); `governance/evidence-architecture.md` §3 mode table + §5 scale-tagging bullet (applied); `scripts/generate/pilot_renderings.py` OT-role string + its generated `working/pilot/pilot-renderings.html` (follow-up, flagged below); `data/decisions/decision_register.yaml` seeded records (historical, left as-is — see item 5).
