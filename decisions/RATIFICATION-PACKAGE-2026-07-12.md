...
**RESOLVED: ratified in full by owner directive 2026-07-13 ("resolve all accept ratify all commit all") — see `decisions/RATIFICATION-RECORD-2026-07-13.md`; execution tracked in `workplan/ratification-execution-register-2026-07-13.md`.**

**Purpose:** one memo, one sitting. Every PROPOSED decision record in the repository (all **16** — RECOUNTED: status-header grep over `decisions/DR-*.md` returns 17 PROPOSED files = these 16 DRs + the unification impact appendix, which rides with A5: 8 May-era + the 3 PR-#2 keystones + the unification DR + 2 via PR #3 + the genealogy/handshake DR + the integrity-protocol DR; none omitted — the body's 16 items are A1–A7 + B1–B9), grouped, each with a one-paragraph statement, a recommendation, what changes on ratification, and the one-line reply that ratifies it. Reply in any form you like; a session directive quoting the item ID with "ratified" / "ratified with amendment: …" / "deferred" is sufficient and will be recorded per decision-capture protocol.
...
The adjudication engine for best-practice-vs-consensus, made computable: genealogy fields on the B6 extraction substrate (root/echo, measurement paradigm, device class — so 1800mm's zero in-corpus roots vs 2440/2400mm's two independent roots becomes a query, and static turning circles can never corroborate swept-path envelopes); the function↔population handshake (`derivation_paths` on determinations, the FDA population↔ICF mapping promoted to data with a named vocabulary-reconciliation step, FDR CONTRADICTS and FDA UNLINKED/MISLINKED as engine gates with a named resolution path), with the **cultural-claim protection** as a named commitment — community-rooted claims (bounded to Co-1/participatory provenance, so the exemption cannot be self-declared) never require mechanical reduction; and the tri-format harvest pipeline (md authored → DB queryable → yaml/json loaded-or-retired). Adversarially reviewed 2026-07-13 (23 findings, all corrected — see the DR's revision history; the mining dossier is preserved at `sessions/session_2026-07-13-value-genealogy-mining-dossier.md`). Companion demonstration: `references/methodology/value-genealogy-worked-example-corridor-width.md` (the full E-08 genealogy, per-population fan-out across all 22 codes, and the integrity findings below).
...
### A7. `DR-2026-07-13-integrity-protocol-three-modes` (added on your three-modes directive)
The adversarial-pass lessons institutionalized: **P1** preventative prior structuring (typed assertions with warrant classes; caveats welded to values; verifiers born with selftests; the self-application clause; sources committed before distillation); **P2** the live claims docket (`scripts/audit/claims_docket.py` — scans working diffs for the empirically-derived defect classes and gates phase commits on warrant annotations; selftest-verified; its demo run over this very branch found 228 triggered claims including the exact "all 14" absolute the second review caught by hand); **P3** the post-processing gate — no ratification request while an adversarial pass is unrun or its findings unapplied, with the pass's required structure specified (fresh context, brief to break, presumed defect, re-run everything, audit the author's chat claims too). Codified in `skills/integrity-protocol_SKILL.md`. The DR practices Mode 1 on itself (its absolutes carry RECOUNTED; its adoption bet is marked PREDICTED with a falsification condition).
**Recommendation:** ratify P1–P3; P3's hard gate is the substantive call — it trades session speed for the only mechanism that has caught what self-checks missed, both times. **Reply:** "A7 ratified" or per item.

**Roster note (RECOUNTED by grep this turn):** with A7 the package covers **16** PROPOSED decision records — A1–A7 + B1–B9; the unification impact appendix rides with A5.
