# Search Log — deaf-classroom-reverberation-time

```yaml
slug: deaf-classroom-reverberation-time
query: "classroom reverberation time RT60 deaf hearing impairment school acoustic design"
last_searched: 2026-04-23 09:30
early_close_triggered: false

languages:
  EN: {status: SEARCHED, results: 14, db: [PubMed, web, ANSI, BB93], co1_pass: not-run, native_standards_pass: complete}
  DE: {status: SEARCHED, results: 3, db: [web, DIN], co1_pass: not-run, native_standards_pass: complete}
  NO: {status: SEARCHED, results: 2, db: [web, SINTEF], co1_pass: not-run, native_standards_pass: complete}
  SV: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  DA: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  FI: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  FR: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  ZH: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  JA: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  NL: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  ES: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  PT: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  KO: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  IT: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}

jurisdiction_coverage:
  US: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "ANSI/ASA S12.60-2010/Part 1 — RT ≤0.6s (<283m³), ≤0.7s (283-566m³), background ≤35dBA. For hearing-impaired children: RT ≤0.3s. Now adopted into IBC 2016 via A117.1. Voluntary unless referenced by code."}
  UK: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "BB93 (Building Bulletin 93) — Acoustic design of schools. MANDATORY in England via Building Regulations. RT targets by room type. Also BS 8233:2014 guidance. 35% of UK classrooms fail BS 8233."}
  AU: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "AS/NZS 2107:2016 recommended design sound levels. NCC references acoustic provisions."}
  CA: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "CSA Z412 applicable. Provincial variations. ANSI S12.60 referenced by some provinces."}
  DE: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "DIN 18041:2016 Acoustic quality in rooms — classroom RT targets. Mandatory where referenced by Landesbauordnungen."}
  NO: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "SINTEF Byggforsk 87-school study — primary cross-language evidence. NS 8175 acoustic classification. TEK17 references."}
  SE: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "BBR acoustic provisions for schools. SS-EN ISO 3382 adopted."}
  ISO: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "ISO 3382-2:2008 RT measurement methodology. ISO 3382-1 for performance spaces."}
  EU: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "EN 17210 references acoustic environment. No specific classroom RT in EN 17210."}
  FR: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "Arrêté du 25 avril 2003 includes school acoustic requirements."}
  CH: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false, note: "SIA 181 acoustic protection — school provisions likely. Not verified."}
  NL: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  DK: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false, note: "BR18 acoustic class system. DS 490 acoustic classification."}
  FI: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false, note: "SFS 5907 acoustic classification."}
  JP: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  SG: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  NZ: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "AS/NZS 2107 shared with AU."}
  KR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  BR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ES: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  IT: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  PT: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  IE: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CN: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  IN: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ZA: {status: NOT-RUN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  MX: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CL: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ID: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  BD: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  NG: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  PH: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  EG: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  KE: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  TH: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CO: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  AR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  PE: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  GT: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  EC: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  UY: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  MA: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  GH: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  TZ: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ET: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}

jurisdiction_summary: >
  STRONG: US (ANSI S12.60 — ≤0.6s general, ≤0.3s hearing-impaired; in IBC via A117.1),
  UK (BB93 mandatory), DE (DIN 18041), NO (SINTEF 87-school study + NS 8175).
  Key divergence: US specifies separate RT for hearing-impaired (0.3s vs 0.6s general).
  BB93 (UK) is most prescriptive mandatory standard for school acoustics.
  SINTEF (NO) provides largest empirical dataset (87 schools).
```
