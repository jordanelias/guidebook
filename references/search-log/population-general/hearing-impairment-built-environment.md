# Search Log — hearing-impairment-built-environment

```yaml
slug: hearing-impairment-built-environment
query: "deaf hearing impairment built environment design acoustic visual alerting DeafSpace"
last_searched: 2026-04-23 10:45
early_close_triggered: false

languages:
  EN: {status: SEARCHED, results: 12, db: [PubMed, web, Gallaudet DeafSpace], co1_pass: partial, native_standards_pass: complete}
  DE: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  SV: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  NO: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  DA: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  FI: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  FR: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  ZH: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  JA: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  NL: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  ES: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  PT: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  KO: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  IT: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}

jurisdiction_coverage:
  US: {status: SEARCHED, co1_attempted: true, tier5_attempted: true, tier6_attempted: true, note: "ADA: visual alerting, assistive listening (§219, §702). Gallaudet University DeafSpace design principles (Bauman & Murray). HLAA advocacy. NAD policy. FCC relay services. ICC A117.1 hearing-aid compatibility."}
  UK: {status: SEARCHED, co1_attempted: true, tier5_attempted: true, tier6_attempted: true, note: "BS 8300: hearing enhancement, visual alerting. Equality Act 2010. RNID (formerly Action on Hearing Loss). BS 5839-1 visual fire alarm. PAS 6463 references acoustic environment for hearing device users."}
  AU: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "NCC hearing augmentation provisions. AS 1428.1 visual alerting. AS 60118.4 hearing loop. Deaf Australia advocacy."}
  CA: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "CSA B651 hearing-related provisions. CHHA advocacy. CHS (Canadian Hearing Society)."}
  DE: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "DIN 18040 Two-Senses principle — visual + auditory redundancy for all safety-critical information. DIN 18041 acoustic design. Deutscher Gehörlosen-Bund advocacy."}
  NO: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "TEK17 visual alerting provisions. Norges Døveforbund advocacy."}
  SE: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "BBR visual alerting. SDR (Sveriges Dövas Riksförbund) advocacy."}
  ISO: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "ISO 21542 hearing-related provisions. IEC 60118-4 hearing loops. IEC TR 63079 code of practice. ISO 24505 universal design. WFD (World Federation of the Deaf) CRPD position."}
  EU: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "EN 17210 hearing enhancement provisions. EN 54-23 visual alarm. EAA (EU 2019/882) communication accessibility. EUD (European Union of the Deaf) advocacy."}
  FR: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "French building code requires hearing enhancement in assembly spaces. FNSF, UNISDA advocacy."}
  CH: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  NL: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "Dovenschap advocacy. NEN 9120 references."}
  DK: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  FI: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  JP: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "全日本ろうあ連盟 advocacy. MLIT Barrier-Free Act hearing provisions."}
  SG: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: true, note: "SADeaf advocacy. BCA Code hearing provisions."}
  NZ: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false, note: "Deaf Aotearoa advocacy."}
  KR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  BR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ES: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  IT: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  PT: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  IE: {status: NO-DATA, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CN: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  IN: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ZA: {status: NOT-RUN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  MX: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CL: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ID: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  BD: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  NG: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  PH: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  EG: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  KE: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  TH: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CO: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  AR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  PE: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  GT: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  EC: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  UY: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  MA: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  GH: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  TZ: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ET: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}

jurisdiction_summary: >
  DEAF built environment provisions span: visual alerting (all P1 codes), hearing loops/assistive
  listening (most P1 codes), acoustic design (DE DIN 18041, UK BB93 for schools), DeafSpace
  principles (US — Gallaudet). DE Two-Senses principle is the most comprehensive mandatory
  framework — requires visual + auditory redundancy for ALL safety-critical information.
  Cross-references: A-10/A-11 (assistive listening), B-10 (visual alarm), D-10 (transparent
  glazing for sign language visibility), deaf-spatial-design BPC, deaf-acoustic-built-environment BPC.
```
