slug: threshold-door-hardware
query: "accessible door threshold height hardware lever handle operating force"
last_searched: 2026-03-27 00:45
co1_pass_date: 2026-03-30 19:10
early_close_triggered: false

native_aliases:
  SV: tröskel / dörrbeslag [INLINE-GENERATED]
  NO: terskel / dørvridere [INLINE-GENERATED]
  DA: tærskel / dørgreb [INLINE-GENERATED]
  FI: kynnys / ovenkahva [INLINE-GENERATED]
  FR: seuil / quincaillerie de porte [INLINE-GENERATED]
  DE: Schwelle / Türbeschlag / Drücker [INLINE-GENERATED]
  ZH: 门槛 / 门把手 [INLINE-GENERATED]
  JA: 敷居 / ドアハンドル [INLINE-GENERATED]
  NL: drempel / deurbeslag [INLINE-GENERATED]
  ES: umbral / herrajes de puerta [INLINE-GENERATED]
  PT: soleira / ferragem de porta [INLINE-GENERATED]
  KO: 문턱 / 문손잡이 [INLINE-GENERATED]
  IT: soglia / ferramenta per porte [INLINE-GENERATED]
concept_boundary_warnings:
  - JA: 敷居 is traditional fusuma/shoji rail; also search バリアフリー + 出入口
  - ZH: 无障碍 is standard accessibility term; search 无障碍出入口
  - KR: 장애인편의 + 출입구 preferred over 무장애

languages:
  EN: {status: SEARCHED, results: 18, db: [web, PubMed], co1_pass: complete, native_standards_pass: complete}
  DE: {status: SEARCHED, results: 6, db: [web], co1_pass: not-run, native_standards_pass: complete}
  SV: {status: SEARCHED, results: 4, db: [web], co1_pass: not-run, native_standards_pass: complete}
  NO: {status: SEARCHED, results: 4, db: [web], co1_pass: not-run, native_standards_pass: complete}
  DA: {status: SEARCHED, results: 2, db: [training-knowledge], co1_pass: not-run, native_standards_pass: partial}
  FI: {status: SEARCHED, results: 2, db: [training-knowledge], co1_pass: not-run, native_standards_pass: partial}
  FR: {status: SEARCHED, results: 5, db: [web], co1_pass: not-run, native_standards_pass: complete}
  ZH: {status: SEARCHED, results: 2, db: [training-knowledge], co1_pass: not-run, native_standards_pass: partial}
  JA: {status: SEARCHED, results: 2, db: [training-knowledge], co1_pass: not-run, native_standards_pass: partial}
  NL: {status: SEARCHED, results: 2, db: [training-knowledge], co1_pass: not-run, native_standards_pass: partial}
  ES: {status: SEARCHED, results: 2, db: [training-knowledge], co1_pass: not-run, native_standards_pass: partial}
  PT: {status: SEARCHED, results: 2, db: [training-knowledge], co1_pass: not-run, native_standards_pass: partial}
  KO: {status: SEARCHED, results: 2, db: [training-knowledge], co1_pass: not-run, native_standards_pass: partial}
  IT: {status: SEARCHED, results: 2, db: [training-knowledge], co1_pass: not-run, native_standards_pass: partial}

jurisdiction_coverage:
  US: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "ADA 2010 §404.2.7: door hardware operable without tight grasp/twist. Lever handles required. Force ≤22N (interior). §309: operable parts."}
  UK: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "BS 8300-2:2018 §8: lever handles, D-pulls. Opening force ≤30N (BS) / ≤20N (best practice). AD M references."}
  AU: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "AS 1428.1:2021 §13: D-type or lever handles. Opening force ≤20N."}
  CA: {status: SEARCHED, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  DE: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "DIN 18040-1/2: lever handles 850mm. Opening force not specified in DIN 18040 (referenced in ASR A1.7 for workplaces)."}
  NO: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "TEK17 §12-15: handles operable with one hand."}
  SE: {status: SEARCHED, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  ISO: {status: SEARCHED, co1_attempted: false, tier5_attempted: true, tier6_attempted: true, note: "ISO 21542:2021 §14.5: lever handles or equivalent. Opening force ≤25N preferred."}
  EU: {status: SEARCHED, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  FR: {status: SEARCHED, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  CH: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  NL: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  DK: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  FI: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  JP: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  SG: {status: SEARCHED, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  NZ: {status: SEARCHED, co1_attempted: false, tier5_attempted: false, tier6_attempted: true}
  KR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  BR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ES: {status: SEARCHED, results: 2, db: [web], co1_pass: partial, native_standards_pass: partial}
  IT: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  PT: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  IE: {status: NOT-RUN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CN: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  IN: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ZA: {status: NOT-RUN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  MX: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CL: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ID: {status: THIN, results: 1, db: [web], co1_pass: not-run, native_standards_pass: partial}
  BD: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  NG: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  PH: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  EG: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  KE: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  TH: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  CO: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  AR: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  PE: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  GT: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  EC: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  UY: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  MA: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  GH: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  TZ: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}
  ET: {status: THIN, co1_attempted: false, tier5_attempted: false, tier6_attempted: false}

jurisdiction_summary: >
  KEY DIVERGENCE: opening force — AU ≤20N (strictest) vs US ≤22N (interior) vs UK ≤30N (BS) / ≤20N (best practice) vs ISO ≤25N. Handle height — DE 850mm vs general 900-1100mm.

```
