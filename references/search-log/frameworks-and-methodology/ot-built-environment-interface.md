# Search Log — ot-built-environment-interface

```yaml
slug: ot-built-environment-interface
query: "OT built environment interface occupational therapy home modification assessment"
...
jurisdiction_summary: >
  OT-built-environment interface is governed by OT professional frameworks (CAOT, AOTA, RCOT, WFOT) rather than building codes. No jurisdiction has a building code that mandates OT involvement in design. UK RCOT Housing Adaptations Without Delay and RCOTSSH GenHOME are Tier Co-2 sources. Cross-reference Part 9 (Working with Specialist Consultants).
```
