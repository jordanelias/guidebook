...
| Best-practice ≠ convergence — retained as the weak-band honesty rule; T4–6 anchor at graded (weak) strength, never unflagged | `governance/tier-system.md` §3 (superseded banner) + §8; `decisions/DR-2026-07-21-evidence-architecture-option-a-execution.md` |
...
- **I3 (amended 2026-07-21, Option A — absolute form repealed) — No *unflagged* and no *above-weak-band* best-practice language on regulatory-stratum-only cells,** in any register, under any weighting profile: every best-practice rendering of such a cell must carry the weak/code-derived flag and the convergence-not-evidence caveat, and may not exceed the register map's ○ row (I5 applies with that row as ceiling). `[ENGINE-LAG → DR-2026-07-21 §5: scripts/audit/register_integrity_check.py still enforces the absolute form and would fail compliant weak-band renderings; its lexicon check and tamper injections must be reworked before any RSO cell renders.]`
