...
- **952 source-instances** are linked across **79 of 79 slices**; **0 slices carry zero linked evidence**.
- **Grade distribution:** A=11 · B=28 · C=25 · D=12 · E=3 · F=0  (A≥80, B≥65, C≥50, D≥35, E>0, F=empty).
- **Tier profile is code-and-clinical heavy, synthesis-light.** Of linked instances: T1=93, T2=115, T3=352, T4=86, T5=155, T6=151. Only **115 Tier-2 (systematic-review / evidence-based-standard) instances** exist across the whole corpus — the synthesis tier that best anchors best-practice claims is the thinnest.
- **Anchoring strength, banded.** Under the weighted-strength model (§8) every tier can anchor a best-practice claim, weighted by tier: **490/952 (51%)** of instances anchor at ● full strength (T1/Co-1/T2/Co-2/T3-clinical, adjudicated), 241 at ◐ partial (T4/T5 standards practice), 211 at ○ weak (T3-grey/T6/grey floor). **10 DISPUTED instances anchor at no band** (§4). By slice: **67 full · 12 partial · 0 weak-only** (of 79 evidenced). Every evidenced slice anchors at ● full or ◐ partial strength — none rests on a weak-only base.
- **Anglophone concentration is the dominant quality risk.** **720/952 (76%) of linked sources are English-language**; only 232 are non-English. By jurisdiction, 353 instances are native-Anglophone (US/UK/AU/CA/NZ/IE), 213 supranational (INT/EU/ISO), 309 other, 77 unrecorded.
...
**Slice = slug.** The 79 ACTIVE slugs are the unit of audit. Evidence is attributed through `source_slug_links`; each linked `evidence_sources` row is one *source-instance* (a source shared by two slices counts once in each). The 952 instances collapse to **780 unique sources** (reuse factor 1.22×; 125 sources span >1 slice, one — `REF-00050` — spans 6). Instance-weighting is deliberate — it measures per-slice coverage — but shared sources are re-counted, so corpus tier/language totals read ~22% above unique-source counts. (24 of the 804 rows in `evidence_sources` are linked to no active slug.)
...
**Strength-band split of instances:** **490/952 (51%)** anchor at ● full (T1/Co-1/T2/Co-2/T3-clinical), 241 (25%) at ◐ partial (T4/T5 standards), and 211 (22%) at ○ weak (T3-grey/T6/grey floor); a further 10 DISPUTED instances anchor at no band (§4). No evidenced slice rests on a ○ weak-only base — every slice anchors at ● full or ◐ partial strength (see the band breakdown in §4).

### (3) Jurisdictions sourced
Distinct jurisdiction strings across the corpus: **50**, none mis-filed as language codes in the `jurisdiction` column. Top: INT (200), US (150), UK (99), DE (50), AU (46), CA (33), JP (30), NL (27), NO (27), SE (20).
...
- **Jurisdiction axis (all 952 instances):** native-Anglophone (US/UK/AU/CA/NZ/IE) **353** · supranational/English-medium (INT/EU/ISO) **213** · English-official + other non-Anglophone **309** · **no jurisdiction recorded 77**. (These four sum to 952 = all instances.)
...
| D | 12 | weak — few sources / single jurisdiction / English-only |
...
| 35 | **B** | 66.4 | `therapeutic-lighting-design` | sensory-environment | 8 | ● | 4 | 4 | 0 | 0 | T2×2,T3×2,T4×1,T5×3 | 4 | 2 | 87.5 | 0.0 | 16·24.0·13·5·8.4 |
...
| 39 | **B** | 65.6 | `upper-limb-impairment-built-environment` | population-general | 17 | ● | 16 | 0 | 1 | 0 | T1×1,T2×2,T3×14 | 4 | 1 | 100.0 | 50.0 | 20·28.8·13·0·3.8 |
...
| 65 | **D** | 49.8 | `ofs-built-environment` | health-and-symptom-management | 7 | ● | 4 | 2 | 1 | 0 | T1×3,T3×1,T5×2,T6×1 | 3 | 1 | 100.0 | 85.7 | 12·23.7·13·0·1.1 |
| 66 | **D** | 49.6 | `ot-cpg-built-environment` | population-general | 6 | ● | 4 | 0 | 0 | 2 | T2×6 | 4 | 1 | 100.0 | 83.3 | 12·23.3·13·0·1.3 |
...
| 68 | **D** | 48.7 | `luminance-contrast-lrv-evidence-base` | wayfinding-and-signage | 7 | ● | 4 | 2 | 1 | 0 | T3×4,T5×2,T6×1 | 4 | 1 | 100.0 | 100.0 | 12·23.7·13·0·0.0 |
...
- **Instance-weighted, not source-weighted.** The 952 instances are 780 unique sources, so corpus tier/language totals run ~22% above unique-source counts. Per-slice figures are unaffected.
