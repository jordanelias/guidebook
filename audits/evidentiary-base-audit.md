...
- **973 source-instances** are linked across **80 of 80 slices**; **0 slices carry zero linked evidence**.
- **Grade distribution:** A=11 · B=30 · C=26 · D=10 · E=3 · F=0  (A≥80, B≥65, C≥50, D≥35, E>0, F=empty).
- **Tier profile is code-and-clinical heavy, synthesis-light.** Of linked instances: T1=98, T2=109, T3=369, T4=83, T5=170, T6=144. Only **109 Tier-2 (systematic-review / evidence-based-standard) instances** exist across the whole corpus — the synthesis tier that best anchors best-practice claims is the thinnest.
- **Anchoring strength, banded.** Under the weighted-strength model (§8) every tier can anchor a best-practice claim, weighted by tier: **503/973 (52%)** of instances anchor at ● full strength (T1/Co-1/T2/Co-2/T3-clinical, adjudicated), 253 at ◐ partial (T4/T5 standards practice), 207 at ○ weak (T3-grey/T6/grey floor). **10 DISPUTED instances anchor at no band** (§4). By slice: **69 full · 11 partial · 0 weak-only** (of 80 evidenced). Every evidenced slice anchors at ● full or ◐ partial strength — none rests on a weak-only base.
- **Anglophone concentration is the dominant quality risk.** **725/973 (75%) of linked sources are English-language**; only 248 are non-English. By jurisdiction, 369 instances are native-Anglophone (US/UK/AU/CA/NZ/IE), 206 supranational (INT/EU/ISO), 313 other, 85 unrecorded.
...
**Slice = slug.** The 80 ACTIVE slugs are the unit of audit. Evidence is attributed through `source_slug_links`; each linked `evidence_sources` row is one *source-instance* (a source shared by two slices counts once in each). The 973 instances collapse to **764 unique sources** (reuse factor 1.27×; 140 sources span >1 slice, one — `REF-00323` — spans 7). Instance-weighting is deliberate — it measures per-slice coverage — but shared sources are re-counted, so corpus tier/language totals read ~27% above unique-source counts. (65 of the 829 rows in `evidence_sources` are linked to no active slug.)
...
**Strength-band split of instances:** **503/973 (52%)** anchor at ● full (T1/Co-1/T2/Co-2/T3-clinical), 253 (26%) at ◐ partial (T4/T5 standards), and 207 (21%) at ○ weak (T3-grey/T6/grey floor); a further 10 DISPUTED instances anchor at no band (§4). No evidenced slice rests on a ○ weak-only base — every slice anchors at ● full or ◐ partial strength (see the band breakdown in §4).

### (3) Jurisdictions sourced
Distinct jurisdiction strings across the corpus: **53**, none mis-filed as language codes in the `jurisdiction` column. Top: INT (193), US (154), UK (101), DE (50), AU (48), CA (39), JP (30), NL (27), NO (27), SE (20).
...
- **Jurisdiction axis (all 973 instances):** native-Anglophone (US/UK/AU/CA/NZ/IE) **369** · supranational/English-medium (INT/EU/ISO) **206** · English-official + other non-Anglophone **313** · **no jurisdiction recorded 85**. (These four sum to 973 = all instances.)
...
- **Instance-weighted, not source-weighted.** The 973 instances are 764 unique sources, so corpus tier/language totals run ~27% above unique-source counts. Per-slice figures are unaffected.
