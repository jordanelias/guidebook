...
- The values taken from each source (per lens, per jurisdiction). An extraction states at least one of four lenses (D-0182): the **population** column carries the identity lens with the free-text setting beneath it, and **other lenses** carries the ICF, access-need and medical ones in a column of their own.
...
| Per-source extraction | `source_value_extractions` *(added migration 018; re-keyed migration 073)* | "This source asserts value X for **parameter P** — the registry row, `base_parameters.parameter_id`, whose name is reached through `terms` — stated in at least one of four **lenses** (identity / ICF / access-need / medical), jurisdiction J" |
