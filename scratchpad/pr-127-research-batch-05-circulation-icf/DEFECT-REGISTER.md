...
| D05-030 | PROCESS | P2 | OPEN, OWNER-GATED — `transcripts/` is 17 MB of JSONL visible to ripgrep and will flood ordinary searches, which is the exact problem `.ignore` exists to solve. Adding an entry is owner-gated by `decisions/DR-2026-08-06-cold-storage-search-scope.md`, so none was added. The same file's own reasoning applies (frozen records whose hits answer current questions wrongly) and its stated exception may too (a transcript answers "what did the agent actually do" with a TRUE hit). Owner's call. |
