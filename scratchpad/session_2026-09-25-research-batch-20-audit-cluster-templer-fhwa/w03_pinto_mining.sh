...
python3 scripts/db.py log-search --slug accessible-circulation-geometry --language EN --engine crossref-deposit --depth-method scoping --session "$S" \
...
EXEC=$(python3 /tmp/claude-0/-home-user-guidebook/34e8c762-53ff-5f39-b746-3f5298058fcb/scratchpad/q.py --db "$GUIDEBOOK_DB_PATH" "select max(exec_id) from search_executions" | sed -n 2p)
