...
# RULE 3: NEVER WRITE data/guidebook.db DIRECTLY. Verified that scripts/db.py has no
# guard on the canonical path, and scratchpad/pr-143-.../replay-batch-10.sh -- the first
# replay script in this repo -- protected against it by creating the scratch DB itself.
# Batches 13 and 14 dropped that and only asserted the variable was SET, so
# `GUIDEBOOK_DB_PATH=data/guidebook.db ./replay.sh` would have written the canonical blob.
if [ "$(readlink -f "$GUIDEBOOK_DB_PATH")" = "$(readlink -f data/guidebook.db)" ]; then
  echo "REFUSING: GUIDEBOOK_DB_PATH resolves to the canonical data/guidebook.db." >&2
  echo "  cp data/guidebook.db \"\$SCRATCH/scratch.db\" && export GUIDEBOOK_DB_PATH=\"\$SCRATCH/scratch.db\"" >&2
  exit 1
fi
# The --session trailer is a curated fact repeated on every call; the wrapper knows it
# once. Convention taken from replay-batch-10.sh:15, which this pair should have followed.
db(){ python3 scripts/db.py "$@" --session "$SID"; }
PRIOR='PRIORS.md (committed 6893397, before any query)'

# ---- 1. searches --------------------------------------------------------------
db log-search --slug $SLUG --language EN --jurisdiction INT \
...
# ---- 2. the admission ---------------------------------------------------------
db add-source --ref-id REF-00998 \
...
db log-search --slug $SLUG --language EN \
...
db add-extraction --ref-id REF-00998 --slug $SLUG --parameter-id 3 \
...
db add-population-match --ref-id REF-00998 --target-population MOB \
...
db observe-term --ref-id REF-00998 --surface-form 'Class 1 and 2 buildings' --language EN \
...
db observe-term --ref-id REF-00998 --surface-form 'Livable Housing Design Standard' --language EN \
...
db observe-term --ref-id REF-00998 --surface-form 'private dwellings' --language EN \
...
db add-code-lead --jurisdiction AU --standard-name 'AS 1428.1:2021 — ramp gradients, landings and handrails' \
...
db add-code-lead --jurisdiction AU --standard-name 'Livable Housing Design Standard (NCC, 2022)' \
 --status REFERENCE-ONLY \
...
db add-economics-entry --entry-id ECON-001 --pillar construction --entry-type research_gap \
...
# ---- 4. candidates -------------------------------------------------------------
db add-candidate --found-under-slug $SLUG --disposition MISCELLANEOUS \
...
db add-candidate --found-under-slug $SLUG --disposition PENDING-VERIFICATION \
...
db retire-specification --specification-id 6 \
